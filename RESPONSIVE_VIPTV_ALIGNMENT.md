...
6. **The guide has no channel paging controls.** `Previous channels` / `Next channels` are removed from the responsive guide; the category sidebar is the intended channel-narrowing control. Reaching the end of the loaded channel rows loads the next API page inside the same scroll, so a category larger than one page stays reachable without buttons. TV remote paging remains unchanged.
7. **Route changes must not move the frame or the header.** The application scroller reserves its scrollbar gutter, so navigating between a tall route (Home) and a short one (Search, Live TV, My List) cannot shift the content sideways; the guide's own timeline scroller does the same for categories whose row count crosses the guide height. The header's profile control occupies a fixed slot (144px at 900px and above, 48px below), so a late-loading avatar, a changed profile name or a missing profile control cannot reflow the brand, navigation or Watch on TV actions.

Acceptance: navigating Home to Discover, Search and Live TV leaves the brand, navigation, cast and profile controls and the content heading at identical coordinates, and the reserved gutter is unchanged. Channel rows beyond the first API page appear by scrolling, with the loaded count reported as text only.
