...
Status: owner-directed implementation specification for the responsive study. Supersedes the presentation choices in RUI-014–017 where they differ below. Existing TV/Roku app behavior is unchanged; no native or production adoption is implied. Tracking: [design #8](https://github.com/viptv-org/design/issues/8).
...
4. **Apply across the study.** Home, detail/episodes, menus/sources, search/list/live, profiles, settings, player and TV-controller controls share this visual system. Import original design-owned icons/avatars with a path/hash manifest. New actions lacking a canonical icon may retain a simple neutral supplementary glyph; do not pretend there is a source asset for them.
